...
render_stock <- function(state, theme, session_theme_dir) {
  ...
  if (length(state$stock) > 0) {
    stock_asset <- find_asset(card_id = "stock", back = FALSE, theme = theme, session_theme_dir = session_theme_dir)
    if (!is.null(stock_asset)) {
      return(div(
        class = "stock-stack",
        onclick = if (clickable) "Shiny.setInputValue('card_click','stock',{priority:'event'})" else NULL,
        div(class = "sol-card stock-shadow-card"),
        div(class = "sol-card stock-shadow-card offset-1"),
        div(class = "sol-card stock-shadow-card offset-2"),
        div(class = "sol-card stock-top-card", img(src = stock_asset, class = "card-img", draggable = "false"))
      ))
    }
    return(div(
      class = "stock-stack",
      ...
      div(class = "sol-card stock-shadow-card offset-1"),
      div(class = "sol-card stock-shadow-card offset-2"),
      card_tag(face_up = FALSE, theme = theme, session_theme_dir = session_theme_dir, extra_classes = "stock-top-card")
    ))
  }
  div(
    class = "recycle-wrap",
...
render_waste <- function(state, theme, session_theme_dir) {
  ...
  visible_n <- min(length(state$waste), state$draw_n)
  visible_ids <- tail(state$waste, visible_n)

  layers <- lapply(seq_along(visible_ids), function(i) {
    card_id <- visible_ids[[i]]
    is_top <- i == length(visible_ids)
    div(
      class = paste("waste-layer", if (is_top) "top-layer" else "under-layer"),
      style = sprintf("left:%dpx;", (i - 1) * 26),
      card_tag(
        card_id = card_id,
        face_up = TRUE,
        clickable = is_top,
        click_value = if (is_top) "waste" else NULL,
        selected = is_top && selected,
        deck = state$deck,
        theme = theme,
        session_theme_dir = session_theme_dir,
        draggable = is_top,
        drag_payload = if (is_top) source_payload("waste") else NULL
      )
    )
  })
  ...
  div(
    class = "waste-fan",
    style = sprintf("width:%dpx;", 100 + (visible_n - 1) * 26),
    layers
  )
}
  ...
  tags$head(
    ...
    tags$style(HTML(" 
      ...
      * { box-sizing: border-box; }
      ...
      .stock-shadow-card.offset-1 { left: 6px; top: 2px; }
      .stock-shadow-card.offset-2 { left: 12px; top: 4px; }
      .stock-top-card { left: 18px; top: 0; }
      .waste-fan {
        position: relative;
      ...
      @media (max-width: 860px) {
        ...
        .stock-waste { justify-content: flex-start; }
        .tableau-row {
          overflow-x: auto;
          display: flex;
          gap: 10px;
  ...
  div(
    ...
    div(
      ...
      div(
        ...
        p(class = "subtitle", "Version Nuées par défaut, inspirée d’un agencement proche du solitaire FRVR et de Windows.")
      ),
      div(
        class = "controlbox",
        div(
        ...
        div(class = "help-text", HTML("Chargement optimisé côté interface : un écran d’attente s’affiche pendant l’initialisation, puis le jeu démarre avec le thème Nuées par défaut."))
      ),
      div(
        class = "statusbox",
    ...
    div(
      ...
      div(class = "stock-waste", uiOutput("waste_ui"), uiOutput("stock_ui"))
    ),
